...
class Scrapper
	...
	def get_townhall_names(department) # ici on récupère les noms
		townhall_names = []
		page = Nokogiri::HTML(open(department))
	...
	def get_townhall_urls(department) # ici on récupère les urls
		townhall_url = []
		page = Nokogiri::HTML(open(department))
	...
	def get_townhall_department(townhall_url) # ici on récupère les emails, grâce aux urls en placés en paramètres
	 ...
	 townhall_url.each do |url_town|
	   page = Nokogiri::HTML(open(url_town))
	   page.xpath("'//main/section[1]/div/div/div/p[1]/a'").each do |department|
	...
	def make_the_hash(townhall_names, townhall_url, townhall_email, townhall_department) # ici on crée le hash final contenant toutes les informations
		hash_final = Hash[townhall_names.zip(townhall_email, townhall_department)]
	  return hash_final
	end
	...
	def perform
		...
		department = "http://annuaire-des-mairies.com/lot-et-garonne.html"
		townhall_names = get_townhall_names(department)
		townhall_url = get_townhall_urls(department)
		townhall_email = get_townhall_email(townhall_url)
		townhall_department = get_townhall_department(townhall_url)
		department = "http://annuaire-des-mairies.com/alpes-maritimes.html"
		townhall_names = get_townhall_names(department)
		townhall_url = get_townhall_urls(department)
		townhall_email = get_townhall_email(townhall_url)
		townhall_department = get_townhall_department(townhall_url)
		department = "http://annuaire-des-mairies.com/hautes-alpes.html"
		townhall_names = get_townhall_names(department)
		townhall_url = get_townhall_urls(department)
		townhall_email = get_townhall_email(townhall_url)
		townhall_department = get_townhall_department(townhall_url)
